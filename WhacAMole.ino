...
void loop()
{
  ...
  if (wrongCounter > 2) {  // Проверяем количество неправильных нажатий
    lcd.clear();  // Выводим на экран количество ошибок
    lcd.setCursor(6, 0);
    lcd.print("MANY");
    lcd.setCursor(5, 1);
    lcd.print("WRONGS");
...
void downloadMenu() { // Загрузка заставочного экрана
...
void gameOver() {  // Заставка при проигрыше
  delay(1000);

  for (int i = 0; i < 3; i++) { // Красивая анимация при проигрыше (моргает и пищит три раза)
    digitalWrite(8, HIGH);  // Подаем питание на светодиод
    digitalWrite(9, HIGH);
    digitalWrite(10, HIGH);
    digitalWrite(11, HIGH);
    tone(BUZZER_PIN, 400, 100);  // Включаем пищалку на 50 миллисекунд
    delay(300);
    digitalWrite(8, LOW);  // Снимаем питание со светодиодов
    digitalWrite(9, LOW);
    digitalWrite(10, LOW);
    digitalWrite(11, LOW);
    delay(300);
  ...
  tone(BUZZER_PIN, 800, 200);  // Включаем пищалку на 200 миллисекунд

  lcd.clear(); // Выводим сообщение о конце игры, число очков и ошибки
  lcd.setCursor(6, 0);
  lcd.print("GAME");
  lcd.setCursor(6, 1);
  lcd.print("OVER");
  ...
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("SCORE:");  // Вывод текущего счета на экран
  lcd.setCursor(6, 0);
  lcd.print(currentScore);
  lcd.setCursor(0, 1);
  lcd.print("WRONGS:");  // Вывод текущих ошибок на экран
  lcd.setCursor(7, 1);
  lcd.print(wrongCounter);
  exit(0);  // Принудительно выходим из цикла игры
}
